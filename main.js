...
await this.setObjectNotExistsAsync("info.apiCalls.limit", {
    type: "state",
    common: { name: "API limit (per hour)", type: "number", role: "value", read: true, write: false },
    native: {}
});
await this.setObjectNotExistsAsync("info.apiCalls.remaining", {
    type: "state",
    common: { name: "API remaining (this hour)", type: "number", role: "value", read: true, write: false },
    native: {}
});
await this.setObjectNotExistsAsync("info.apiCalls.resetAt", {
    type: "state",
    common: { name: "API reset timestamp", type: "string", role: "date", read: true, write: false },
    native: {}
});
await this.setObjectNotExistsAsync("info.apiCalls.todayTotal", {
    type: "state",
    common: { name: "API calls today (sum)", type: "number", role: "value", read: true, write: false },
    native: {}
});
await this.setObjectNotExistsAsync("info.apiCalls.todayDate", {
    type: "state",
    common: { name: "Date of daily counter", type: "string", role: "text", read: true, write: false },
    native: {}
});
...
if (sDate && sDate.val === today && sToday && typeof sToday.val === "number") {
    // Gleiches Datum → weiterzählen
    this.apiCallsToday = sToday.val;
    this.apiCallsDate  = sDate.val;
    this.log.info(`API counter restored → ${this.apiCallsToday} calls so far for ${this.apiCallsDate}`);
} else {
    // Neuer Tag oder keine gespeicherten Werte → neu starten
    this.apiCallsToday = 0;
    this.apiCallsDate  = today;
    await this.setStateAsync("info.apiCalls.todayTotal", { val: 0, ack: true });
...
if (this.updateInterval) {
    ...
    if (this.effectiveConfig.activityrecords) {
        ...
        if (this.effectiveConfig.intraday && this.effectiveConfig.refresh > 0) {
            const interval = Math.max(1, Math.round(this.effectiveConfig.refresh));
...
async getIntradayHeartRate(intervalMinutes = 1) {
    ...
    const resolution = intervalMinutes <= 1 ? "1min"
    ...
    try {
        ...
        if (response.status === 200 && response.data["activities-heart-intraday"]) {
            ...
            if (dataset.length > 0) {
                ...
                if (DEBUG_SLEEP_LOG) {
                    const prev = this.recentHeartData.length > 1
    ...
    if (this.recentHeartData?.length > 0) {
        ...
        this.recentHeartData = this.recentHeartData.filter(p => p.ts.toISOString().startsWith(today));
    }
}
...
async setSleepStates(data) {
    ...
    if (filteredBlocks.length === 0) {
        ...
        if (naps.length > 0) {
            this.dlog("info", `Main sleep ignored, but ${naps.length} nap(s) found → using those.`);
            filteredBlocks = naps;
        } else {
            this.dlog("debug", "All sleep blocks ignored by filters (no naps or main sleep)");
            return false;
        }
    }
    ...
    computeFellAsleepAt(block) {
        ...
        // ---------------------------------------------------------------------------
        if (block.isMainSleep && this.effectiveConfig.intraday && this.recentHeartData?.length > 10) {
            const start = new Date(block.startTime);
            const window30 = this.recentHeartData.filter(p => Math.abs(start - p.ts) <= 30 * 60000);
            if (window30.length > 10) {
                const mean = arr => arr.reduce((a, b) => a + b, 0) / arr.length;
    const smooth = arr => {
        if (!arr || arr.length === 0) return 0;
        if (arr.length <= 3) return mean(arr);
        return mean(arr.slice().sort((a, b) => a - b).slice(1, -1));
    };
                ...
                if (drop < 5) {
                    this.dlog("debug", `Heart-rate drop only ${drop.toFixed(1)} BPM around ${start.toISOString()} → likely not asleep (Couch phase).`);
                    return this._parseISO(block.startTime); // Frühschlaf-Block ignorieren
                } else {
                    this.dlog("debug", `Heart-rate dropped ${drop.toFixed(1)} BPM → likely real sleep.`);
                }
            }
        }
        ...
        if (block.isMainSleep && this.effectiveConfig.ignoreEarlyMainSleepEnabled) {
            ...
            if (startHour < earlyLimit) {
                let firstLight = null, firstDeep = null, firstREM = null;

                for (const s of segs) {
                    if (!firstLight && s.level === "light") firstLight = new Date(s.dateTime);
                    if (!firstDeep && s.level === "deep") firstDeep = new Date(s.dateTime);
                ...
                if (firstReal && firstReal > firstSeg) {
                    this.dlog("debug", `Adjusted early main sleep start from ${firstSeg.toISOString()} → ${firstReal.toISOString()}`);
                    return firstReal;
                }
            }
        }
        ...
        // ---------------------------------------------------------------------------
        this.dlog("debug", "No refined sleep phase found → fallback to block.startTime");
